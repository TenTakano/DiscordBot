defmodule DiscordBot.Llm.OpenAiClientTest do
  ...
  describe "ask_model/2" do
    ...
    test "returns error tuple on timeout" do
      ...
      log =
        capture_log(fn ->
          assert {:error, "APIへの接続でエラーが発生しました"} = OpenAIClient.ask_model([])
        end)

      assert log =~ "OpenAI API transport error: :timeout"
    end

    test "returns error tuple on connection refused" do
      expect(Mock, :post, 1, fn _endpoint, _body ->
      ...
      assert log =~ "OpenAI API transport error: :econnrefused"
    end
  end
end
